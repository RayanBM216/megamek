...
public abstract class Entity extends TurnOrdered
      implements Transporter, Targetable, RoundUpdated, PhaseUpdated, ITechnology, ForceAssignable, CombatRole, Deployable {
     ...
     * this entity occupies. The central hex is important for drawing multi-hex sprites.
     */
     ...
     * distinguish between multiple equi-distant hexes.
     */
    ...
    /**
     * Keeps track of whether or not this Entity has a critically hit radical heat sink. Using a flag will prevent
     * having to iterate over all of the Entity's mounted equipment
     */
    ...
    /**
     * @return The total number of crew available to supplement marines on boarding actions. Includes officers,
     *       enlisted, and bay personnel, but not marines/ba or passengers.
     */
    public int getNCrew() {
        return nCrew;
    }
    ...
    /**
     * Set whether or not the Mek's arms are flipped to the rear. Does not fire the game event, useful for when it's
     * called repeatedly such as during bot turn calculations
     */
    ...
    public int calcElevation(Hex current, Hex next, int assumedElevation, boolean climb, boolean wigeEndClimbPrevious) {
        ...
        if (getMovementMode() == EntityMovementMode.WIGE) {
            ...
            if (next.containsTerrain(Terrains.BUILDING) || current.containsTerrain(Terrains.BUILDING)) {
                ...
                if (((assumedElevation == bldcur) && (climb || isJumpingNow) && (this instanceof Mek)) ||
                          (retVal > bldnex)) {
                    retVal = bldnex;
     ...
     *
     * @return A value indicating whether or not this entity updated its duplicate marker.
     */
     ...
     * <p>
     * Sub-classes are allowed to override this method. The display name is in the format [Chassis] [Model] ([Player
     * Name]).
     */
    ...
    public String getLocationAbbr(int loc) {
        String[] locationAbbreviations = getLocationAbbrs();
    ...
    public int getHeatFiringModifier() {
        ...
        if ((mod > 0) && (getCrew() != null) && hasAbility(OptionsConstants.UNOFF_SOME_LIKE_IT_HOT)) {
            mod--;
        }
     ...
     * Creates a new mount for this equipment and adds it in.
     */
    public Mounted<?> addEquipment(EquipmentType equipmentType, int loc, boolean rearMounted, int baMountLoc, boolean isArmored, boolean isTurreted)
          throws LocationFullException {
        return addEquipment(equipmentType, loc, rearMounted, baMountLoc, isArmored, isTurreted, false, false, false);
    }

    public Mounted<?> addEquipment(EquipmentType equipmentType, int loc, boolean rearMounted, int baMountLoc, boolean isArmored, boolean isTurreted, boolean isSponsonTurreted)
          throws LocationFullException {
        return addEquipment(equipmentType,
              loc,
              rearMounted,
    ...
    public Mounted<?> addEquipment(EquipmentType equipmentType, int loc, boolean rearMounted, int baMountLoc, boolean isArmored, boolean isTurreted, boolean isSponsonTurreted, boolean isPintleTurreted)
          throws LocationFullException {
        return addEquipment(equipmentType,
              loc,
              rearMounted,
    ...
    public Mounted<?> addEquipment(EquipmentType equipmentType, int loc, boolean rearMounted, int baMountLoc, boolean isArmored, boolean isTurreted, boolean isSponsonTurreted, boolean isPintleTurreted, boolean isOmniPodded)
          throws LocationFullException {
     ...
     */
    public Mounted<?> addEquipment(EquipmentType equipmentType, int loc, boolean rearMounted, int baMountLoc, boolean dwpMounted)
          throws LocationFullException {
        Mounted<?> mounted = Mounted.createMounted(this, equipmentType);
        mounted.setBaMountLoc(baMountLoc);
     ...
     * that both the flags given in MiscType as well as the "actual" flags in MiscTypeFlag can be used.
     *
     ...
     *       {@link CriticalSlot#isDestroyed()}. The crit slots tested are only those in location loc except for
    ...
    protected int critStateCount(int type, int index, int loc, Predicate<CriticalSlot> slotState) {
        ...
        for (int i = 0; i < numberOfCriticals; i++) {
            ...
            // compare types. Super-heavies may have two Mounted in each crit
    ...
    public boolean hasLegActuatorCrit() {
        ...
        for (int i = 0; i < locations(); i++) {
            if (locationIsLeg(i)) {
                if ((getBadCriticals(CriticalSlot.TYPE_SYSTEM, Mek.ACTUATOR_HIP, i) > 0) ||
                          (getBadCriticals(CriticalSlot.TYPE_SYSTEM, Mek.ACTUATOR_UPPER_LEG, i) > 0) ||
                          (getBadCriticals(CriticalSlot.TYPE_SYSTEM, Mek.ACTUATOR_LOWER_LEG, i) > 0) ||
                          (getBadCriticals(CriticalSlot.TYPE_SYSTEM, Mek.ACTUATOR_FOOT, i) > 0)) {
                    hasCrit = true;
                    break;
                }
            }
     ...
     *       mode <code>false</code> if the entity does not have angel ecm or it is not in eccm mode or it is damaged.
     */
    public boolean hasActiveECCM() {
        ...
        if ((game.getOptions().booleanOption(OptionsConstants.ADVANCED_TACOPS_ECCM) ||
                   game.getOptions().booleanOption(OptionsConstants.ADVAERORULES_STRATOPS_ECM)) && !isShutDown()) {
            for (MiscMounted m : getMisc()) {
                MiscType type = m.getType();
                // TacOps p. 100 Angle ECM can have 1 ECM and 1 ECCM at the same
                // time
     ...
     * <p>
     * This method takes into account whether the <code>Entity</code> is a C3 Master or a C3 Slave. If it is a C3 Master,
     * it adjusts the number of remaining nodes depending on the presence of other C3 Slaves and whether the <code>Entity</code>
     * can connect to them as part of a company-level command.
     *
    ...
    /**
     * Hits all criticals of the system occupying the specified critical slot. Used, for example, in a gauss rifle
     * capacitor discharge. Does not apply any special effect of hitting the criticals, like ammo explosion.
     */
    ...
    public void newRound(int roundNumber) {
        ...
        // If we are affected by the TSEMP Shutdown effect, we should remove it now, so we can startup during the end
        // phase
    ...
    public PilotingRollData checkGetUp(MoveStep step, EntityMovementType moveType) {
        ...
        if (this instanceof BipedMek) {
            if ((((Mek) this).countBadLegs() >= 1) && (isLocationBad(Mek.LOC_LARM) && isLocationBad(Mek.LOC_RARM))) {
    ...
    public PilotingRollData checkRunningWithDamage(EntityMovementType overallMoveType) {
        ...
        int gyroDamage = getBadCriticals(CriticalSlot.TYPE_SYSTEM, Mek.SYSTEM_GYRO, Mek.LOC_CT);
    ...
    public boolean canLoad(Entity unit, boolean checkElev) {
        ...
        if (!unit.isEnemyOf(this)) {
             ...
             * loaded. It is still possible for a support vee to carry mechanized BA externally and protos in a bay,
    ...
    /**
     * @param forCost Whether this value is being used for cost calculations, in which case dropshuttle bays count as
     *                two collars.
     *
     * @return The number of docking collars
     */
    ...
    /**
     * Returns whether an entity can flee from its current position. Currently returns true if the entity is on the edge
     * of the board.
     */
    ...
    public boolean hasDetectedEntity(Player p) {
        ...
        if ((game == null) || !game.getOptions().booleanOption(OptionsConstants.ADVANCED_TACOPS_SENSORS)) {
            return false;
        }
    ...
    public boolean isSensorReturn(Player spotter) {
        ...
        boolean sensors = (game.getOptions().booleanOption(OptionsConstants.ADVANCED_TACOPS_SENSORS) ||
                                 game.getOptions()
    ...
    public boolean isEligibleForPhysical() {
        ...
        Iterator<Entity> e = game.inGameTWEntities().iterator();
        ...
        if (cover > LosEffects.COVER_NONE) {
            switch (cover) {
                ...
                case LosEffects.COVER_LEFT:
                    if ((location == Mek.LOC_LLEG) || (location == Mek.LOC_LARM) || (location == Mek.LOC_LT)) {
                        return true;
                    }
                    break;
                case LosEffects.COVER_RIGHT:
                    if ((location == Mek.LOC_RLEG) || (location == Mek.LOC_RARM) || (location == Mek.LOC_RT)) {
                        return true;
                    }
                    break;
    ...
    public void destroyLocation(int loc, boolean blownOff) {
        ...
        // all critical slots set as missing. while we're here, if something is mounted in those crits, set it as hit,
    ...
    public boolean isCapitalFighter(boolean lounge) {
        ...
        return game.getOptions().booleanOption(OptionsConstants.ADVAERORULES_STRATOPS_CAPITAL_FIGHTER) && isFighter();
    }
    ...
    public int chooseSide(Coords attackPos, boolean usePrior) {
        // loop through directions and if we have a non-zero vector, then compute the targetsidetable. If we come to
     ...
     * This is implemented in subclasses, do nothing in general
     *
     * @param amount Ammount of Coolant to add
     */
    ...
    public int countPartialRepairs() {
        if ((null == game) || !game.getOptions().booleanOption(OptionsConstants.ADVANCED_STRATOPS_PARTIALREPAIRS)) {
            return 0;
        }
     ...
     * @param vCriticals contains tuple of integer and critical slot. used as output.
     *
     * @return true if there was a masc failure.
     */
    public boolean checkForMASCFailure(MovePath md, Vector<Report> vDesc, HashMap<Integer, List<CriticalSlot>> vCriticals) {
        if (md.hasActiveMASC()) {
            boolean bFailure;

            // If usedMASC is already set, then we've already checked MASC this turn. If we succeded before, return
     ...
     */
    public boolean checkForSuperchargerFailure(MovePath md, Vector<Report> vDesc, HashMap<Integer, List<CriticalSlot>> vCriticals) {
        if (md.hasActiveSupercharger()) {
            boolean bFailure;

            // If usedSupercharger is already set, then we've already checked Supercharger this turn. If we succeded
     ...
     */
    private boolean doMASCOrSuperchargerFailureCheckFor(MiscMounted masc, Vector<Report> vDesc, HashMap<Integer, List<CriticalSlot>> vCriticals) {
        if ((masc != null) && masc.curMode().equals("Armed")) {
            ...
            if ((!isSupercharger && (rollValue < getMASCTarget())) ||
                ...
                if (isSupercharger) {
                        ...
                        for (int i = 0; i < hits; i++) {
                            if (tank instanceof VTOL) {
                                ...
                                if (heavyMovementDamage) {
                                    ...
                                    // HACK: we abuse the criticalslot item to signify the calling function to deal
                                    // movement damage
                    ...
                    // do the damage. random crit on each leg, but MASC is not destroyed
                    for (int loc = 0; loc < locations(); loc++) {
    ...
    /**
     * Are we trapped inside of a destroyed transport? If so we shouldn't count for BV, which is why we have this
     * check.
     */
    ...
    public void loadDefaultCustomWeaponOrder() {
        WeaponOrderHandler.WeaponOrder weapOrder = WeaponOrderHandler.getWeaponOrder(getChassis(), getModel());
    ...
    /**
     * Returns the Mounted that is referred to by the quirkEntry (which must be a weapon quirk). //FIXME: This is very
     * specialized code that is only needed because we need to identify a weapon in the blk/mtf file. It might be better
     * to write the weapon quirk directly to the weapon, removing the need to find it. Note the override in ProtoMek
     * that doesnt use crit slots. For meks, this is challenge as the specific weapon in a location must still be
     * addressed.
     *
    ...
    public int getMaxWeaponRange(boolean targetIsAirborne) {
        ...
        for (WeaponMounted weapon : getWeaponList()) {
            ...
            if (isAirborne()) {
                ...
                range = (game != null && game.getOptions().booleanOption(OptionsConstants.ADVCOMBAT_TACOPS_RANGE) ?
